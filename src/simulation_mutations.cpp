...
float order  = 10.0;   //normal is 11
float orderL = 10.0;
float monthsART = 0.00; //months before ART starts
...
float beta  = 6*(pow(10,-order-3));   //It is 70 normally  70e-9;

//float beta  = 18.0*(pow(10,-order-3));

float disR = 0.00005;    // 0.5 last version  // 4.0 exponential worked very good
//float disR = 1.5;
//float disR = 0.1;
float idmut = monthsART*100;
float dt = 0.01;
float tEnd = 120.00; //tend 800 should be
...
//    std::cout << Vsequence << " V start of function \n";
    ...
    // It was 7.206
    float mu_L = 7.195+25; //death rate of latent cells  last test 7.2
    // Probabilities
...
//            omp_set_num_threads(nthre);
        ...
        //                std::normal_distribution<double> distribution_PR(-1,1);
            ...
            // Create new sequences from not simulated region of LR (quasi-steady state)
            
            //Create mutants with L=1, A=0
            float lpr = 0;
            float a00 = 0;
            for (int k=0; k<4000*pow(10,(orderL-order)); k++){
...
//            		}while(lpr<-3.2);
                ...
                if (idmut < ID_created){
                SEQUENCES.at(6).second.push_back(1);
                }
                else{
                SEQUENCES.at(6).second.push_back(0);
                }
