...
int order = 11;   //normal is 9
float monthsART = 0.4; //months before ART starts
//float monthsART = 0.03;
// T cells parameters
float T = 1.75*(pow(10,order));
float lam_T = 105*(pow(10,order-1)); //70*5000*50*1000
float mu_T  = 0.2*30;
//float beta  = 60*(pow(10,-order-3));   //It is 70 normally  70e-9;
float beta  = 40.0*(pow(10,-order-3));
float disR = 0.00005;
...
void evolve_sequences(float &L_sequence, float &A_sequence, float &V_sequence, float &LXR_sequence, float &Nclones_sequence, float &nLs_sequence, float &nmLs_sequence, float &nmAs_sequence, float &nDs_sequence, float &nEventss_sequence, float beta, float T, std::vector<float> &Rclones, std::vector<float> &Lclones, std::vector<float> &AGclones, std::vector<int> &IDclones, int &COUNTERclones, int threadnum1, float disR){
    ...
    float mu_L = 7.195+25; //death rate of latent cells
    // Probabilities
    float p_L    = 0.05;  // In reality it is 0.02 (one order below)
    float p_def  = 0.002;
    float p_mut  = 0.33;
//    float p_surv = 1;
    // Antigen parameters
...
//    std::exponential_distribution<double> distribution2(disR);    // pR distribution
...
//        react += (nu_L+ANTIGEN.at(k))*LATENT.at(k)*REACTIVATION.at(k);
        react += (ANTIGEN.at(k))*LATENT.at(k)*REACTIVATION.at(k);
...
//        LATENT.at(k) += ((1-2*REACTIVATION.at(k))*(nu_L + ANTIGEN.at(k)) - mu_L)*LATENT.at(k)*dt + sqrt((nu_L+ANTIGEN.at(k)+mu_L)*LATENT.at(k)*dt)*distribution(generator);
    ...
    std::mt19937 gen(std::chrono::high_resolution_clock::now().time_since_epoch().count() + omp_get_thread_num()*1000);
...
//    std::binomial_distribution<> dR(1, p_surv); //probability of surviving after reactivating (needed because of running 1 orders below)
...
//    }
    ...
    if (Asequence > 200){
        ...
        Asequence += react*dt+nAs-mu_A*Asequence*dt + noise_A;
...
//        }
        ...
        do{
            ...
            float action_0 = mu_A*Asequence;
            float action_1 = beta*T*Vsequence;
            float total_rate = action_0 + action_1;
            action_table.push_back(action_0);
            action_table.push_back(action_1);
            std::discrete_distribution<> d_action(action_table.begin(), action_table.end());
            std::exponential_distribution<double> distribution_e(total_rate);
            double dt_hybrid = distribution_e(generator);
            dtime += dt_hybrid;
            if (dtime < dt){ //i need to loop this
                //put the conditional for if dtime is bigger than dt
                //dynamic of virions for in between times
                for (int kk = 0; kk < round(dt_hybrid/dtt); kk++){
                    Vsequence += -c*Vsequence*dtt + gammas*Asequence*dtt + sqrt(c*Vsequence*dtt + gammas*Asequence*dtt)*distribution(generator);
                    if (Vsequence < 0.5){
                        Vsequence = 0;
                    }
                    // I dont substract the number of events in the dynamics as that will be done when that choice is made
                }// finish virion dynamics
                
                //random choie between death of active cell or infection event
                int value_action = d_action(gen);
                switch (value_action){
                    case 0: {
                        // death of active cell
                        Asequence -= 1;
                        if (Asequence < 0.5){
                            Asequence = 0;
                        }
            ...
            //dynamics of virions
                for (int kk = 0; kk < round((dt-dtime+dt_hybrid)/dtt); kk++){
                    Vsequence += -c*Vsequence*dtt + gammas*Asequence*dtt + sqrt(c*Vsequence*dtt + gammas*Asequence*dtt)*distribution(generator);
                    if (Vsequence < 0.5){
                        Vsequence = 0;
                    }
...
//                std::exponential_distribution<double> distribution2(disR); // pR distribution
                std::normal_distribution<double> distribution_PR(-1.0,0.8);  //pR distribution normal
//                std::exponential_distribution<double> distribution2(3.0);
                std::normal_distribution<double> distribution3(0,sqrt(Asq)); //a0 initialization
            
            	//Create mutants with L=1, A=0
                float lpr = 0;
                float a00 = 0;
            	for (int k=0; k<SEQUENCES.at(9).second.at(j); k++){
...
//            omp_set_dynamic(0);
            ...
            FILE *outTOT = fopen("output/totals.csv","a");
                fprintf(outTOT, "%f,%f,%d,%d,%d,%d,%f,%f,%f,%f,%f,%f,%f,%f,%d,%d,%d,%d,%d\n", t, T, nLclones, nMclones, nDclones, nSequences, Ltotal, Atotal, Vtotal, LXRtotal, Lmut, Amut, Vmut, Dtotal, nL, nmL, nmA, nD,nEvents);
            fclose(outTOT);
...
//               omp_set_dynamic(0);
    ...
    std::cout << nthre  << " threads " << duration.count() << "\n";
    std::cout << '\a';
}           //close of main
